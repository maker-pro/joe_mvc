...
class MysqlPdoHelper
{
    ...
    public static function getInstance()
    {
        if (!self::$instance instanceof self) {
            ...
            self::$instance = new self($db_name, $db_host, $db_username, $db_pwd, $db_port);
        }
    ...
    private function __construct($database, $host, $user, $password, $port, $socket = '')
    {
        $this->host = $host;
    ...
    public function connect()
    {
        ...
        if (Conf::get('db_encoding')) {
            $pdo .= ";charset=" . Conf::get('db_encoding');
        }
        ...
        if (Conf::get('time_zone')) {
            $this->pdo->exec("SET time_zone = '" . addslashes(Conf::get('time_zone')) . "'");
        }
    }
    ...
    {
        if (!defined("CACHE_FUNC_DEBUG_MODE")) return;
        if (CACHE_FUNC_DEBUG_MODE == false) return;
        if (!defined("LOG_LOCATION")) return;
        if (!is_dir(LOG_LOCATION)) return;
        ...
        if (is_array($result)) {
            ...
            if (isset($result["backtrace"][0])) {
                $tmparr = explode("/", $result["backtrace"][0]);
                $backtrace = end($tmparr);
            }
